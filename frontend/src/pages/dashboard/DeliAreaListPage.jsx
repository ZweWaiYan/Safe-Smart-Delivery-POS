...
import { fetchDeliArea, createDeliArea, updateDeliArea, deleteDeliArea } from "@/api/deliAreaAPI";
import CrudSelect from "@/widgets/components/crudSelect";
...
export function DeliAreaListPage() {

    const { canAdd, canEdit, canDelete } = usePermission("Deli Car");

    const [controller, dispatch] = useMaterialTailwindController();
    const { fixedNavbar, openSidenav } = controller;

    const [deliAreaList, setDeliAreaList] = useState([]);
    const [formData, setFormData] = useState({ deliCarNo: "", deliCarTownShip: [], deliCarTownShipId: [], deliCarTownShipName: [] });
    ...
    const deliAreaNameRef = useRef(null);
    const crudSelectRef = useRef(null);
    ...
    const filteredList = deliAreaList.
        filter(item => item && item.deliCarNo)
        .filter(({ deliCarNo }) =>
            deliCarNo.toLowerCase().includes(searchQuery.toLowerCase())
        );
    ...
    const handleDropdownChange = (values) => {
        // setFormData({ ...formData, deliCarTownShip: values });
        setFormData({ ...formData, deliCarTownShip: values, deliCarTownShipId: values, });
    };

    const resetForm = () => {
        setFormData({ deliCarNo: "", deliCarTownShip: [] });
        setErrors({ deliCarNo: "", deliCarTownShip: [] });
        setEditId(null);
    }
    ...
    const handleAdd = () => {
        ...
        deliAreaNameRef.current.focus();
    };

    const handleEdit = (id) => {
        const item = deliAreaList.find((d) => d.deliAreaId === id);
        if (!item) return;
        setFormData({ deliCarNo: item.deliCarNo, deliCarTownShipId: item.townShipIds, deliCarTownShipName: item.townShipNames });
        setEditId(id);
    ...
    const handleDelete = async () => {
        try {
            const response = await deleteDeliArea(parseInt(deleteId));
            setDeliAreaList((prev) => prev.filter((c) => c.deliAreaId !== deleteId));
            setDeleteId(null);
    ...
    const handleSubmit = async (e) => {
        try {
            crudSelectRef.current.resetSelection();
            let response;
            if (editId) {
                response = await updateDeliArea(editId, formData);

                if (response.status === 201) {
                    setDeliAreaList((prevList) =>
                        prevList.map((c) =>
                            c.deliAreaId === response.data.deliAreaId ? response.data : c
                        )
                    );
                    setAlertMessage({ msg: response.message, status: response.status });
                    setAlertOpen(true)
                ...
                response = await createDeliArea(formData);
                if (response.status === 201) {
                    setDeliAreaList((prev) => [response.data, ...prev]);
                    setCurrentPage(1);
                    setAlertMessage({ msg: response.message, status: response.status });
                    setAlertOpen(true);
                }
    ...
    const validate = () => {
        let valid = true;
        const newErrors = { deliCarNo: "", deliCarTownShip: "" };

        if (!formData.deliCarNo.trim()) {
            newErrors.deliCarNo = "Deli Car No is required";
            valid = false;
        }
        if (!formData.deliCarTownShip.trim()) {
            newErrors.deliCarTownShip = "TownShip is required";
            valid = false;
        }
        ...
        <>
                    ...
                    <Card>
                        ...
                        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
                            ...
                            <table className="w-full mt-8 min-w-[640px] table-auto">
                                ...
                                <tbody>
                                    {currentRows.map(
                                        ({ deliAreaId, deliCarNo, townShipNames }, key) => {
                                                    ...
                                                    <td className={className}>
                                                        <div className="flex gap-2">
                                                            ...
                                                            {canDelete && (
                                                                <IconButton
                                                                    color="red"
                                                                    onClick={() => openDeleteConfirmDialog(deliAreaId)}
                                                                >
                    ...
                    <div className="flex flex-col gap-5 bg-white p-4 rounded shadow-md">
                        <div className="flex items-center justify-between pb-3 border-b-2">
                            <h2 className="text-lg font-semibold text-gray-800">
                                {editId !== null ? "Edit Deli Car" : "Add New Deli Car"}
                            </h2>
                            <IconButton
                                size="sm"
                                color="red"
                        ...
                        <Input
                            label="Car No"
                            name="deliCarNo"
                            value={formData.deliCarNo}
                            inputRef={deliAreaNameRef}
                        ...
                        />
                        {errors.deliCarNo && (
                            <span className="text-red-500 text-xs">{errors.deliCarNo}</span>
                        )}
                        <div>
                            <CrudSelect
                                ref={crudSelectRef}
                                deliCarTownShipId={formData.deliCarTownShipId}
                                deliCarTownShipName={formData.deliCarTownShipName}
                        ...
                        <Button
                            ...
                            style={{ backgroundColor: '#49D642', color: 'white', fontWeight: 'bold' }}
                        >
                            Submit
                        </Button>
                    </div>
            ...
            <Dialog open={confirmOpen} handler={() => setConfirmOpen(false)} size="xs" className="!max-w-[90%] sm:!max-w-sm">
                ...
                <DialogBody className="text-lg font-semibold text-blue-gray-600">
                    Are you sure you want to delete this Deli Car record?
                </DialogBody>
                <DialogFooter>
...
export default DeliAreaListPage;
